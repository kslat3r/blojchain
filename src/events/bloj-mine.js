  ...
  node.on('bloj:mine', (json) => {
    ...
    if (bloj.prevHash !== minerConfig.genesisHash) {
      ...
      if (bloj.index !== (lastBloj.index + 1)) {
        logger.error('EVENTbloj:mine', 'Bloj received does not increment last index by 1', bloj);

        return;
      }

      if (lastBloj.hash !== bloj.prevHash) {
        logger.error('EVENTbloj:mine', 'Bloj received does not match last bloj hash', bloj);
